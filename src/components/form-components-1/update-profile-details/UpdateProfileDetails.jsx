...
const ProfileForm = () => {
  ...
  const [formData, setFormData] = useState({
    age: profileDetails?.personalDetails?.age,
    dob: profileDetails?.personalDetails?.dob,
    hobbies: profileDetails?.personalDetails?.hobbies,
    interests: profileDetails?.personalDetails?.interests,
    habits: profileDetails?.personalDetails?.habits,
    qualification: profileDetails?.personalDetails?.qualification,
    profilePicture: null,
  });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      {loading ? (
        ...
        <div className="bg-white p-8 rounded-lg shadow-lg w-full h-screen sm:h-max md:w-[70%] ">
          <h1 className="text-2xl font-bold mb-6 text-center">
            Update your profile
          </h1>
          <form
            onSubmit={handleSubmit}
            className="space-y-5 sm:space-y-0 sm:grid gap-5 grid-cols-2"
          >
            <div>
              <input
                type="number"
